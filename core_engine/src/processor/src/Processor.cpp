...
void Processor::saveResultsToCSV(const std::string& filename, const Result* results) {
    ...
    // Write CSV header using the existing printHeader method
    csvFile << OneObjectRow::printHeader() << std::endl;
    
    // Write data rows using the existing print method
    for (unsigned int i = 0; i != results->size(); i++) {
        const OneObjectRow& oor = results->getRow(i);
        csvFile << oor.print() << std::endl;
    }
...
void Processor::writeResult(){
    ...
    saveResultsToCSV("bacterial_colonies.csv", m_result);
